...
**Footer del sidebar, de 7 filas a 4** (más una segunda pasada): Modo oscuro/claro, Deep Work, Papelera, Ajustes (más el badge de plan, que no es interactivo). Se encontró un cuarto duplicado en una revisión posterior — **"Cerrar sesión"** también existía suelto y dentro de Ajustes → Cuenta; sacada la fila suelta, queda solo en Ajustes.
...
**Bug encontrado después de construido, señalado directamente — ✅ arreglado (v0.82):** el menú desplegable se cortaba cuando la fila estaba cerca del final del árbol visible. **Causa técnica confirmada:** el árbol de páginas vive dentro de un contenedor con `overflowY: 'auto'` (la clase `.glenwyn-scroll`, línea ~1671 de `App.jsx`) — cualquier contenido que se extendiera más allá del borde de ese contenedor se recortaba, y el menú "⋯" (antes posicionado `absolute` respecto a la fila) caía justo en esa zona. No era que el footer lo tapara por z-index — se cortaba antes, por el `overflow` del contenedor padre.

**Solución aplicada — opción 1, portal:** el menú ahora se renderiza vía `createPortal` directo en `document.body`, con posición calculada a partir de `getBoundingClientRect()` del botón, en vez de vivir dentro del árbol que scrollea. Nunca más se corta, sin importar cuántas páginas haya en el árbol o qué tan lejos scrolleaste. Se sumó además una apertura inteligente hacia arriba si no hay espacio suficiente debajo en la ventana completa (no solo dentro del sidebar), y el menú se cierra solo si el árbol scrollea mientras está abierto (para no quedar mal alineado con el botón que ya se movió).

### 60. Menú de acciones de página, estilo Notion 🟠 — ✅ Ancho completo y Bloquear página construidos (v0.81)
De las capturas del menú "⋯" de una página en Notion: Copiar enlace, Copiar el contenido de la página, Duplicar, Mover a, Archivar, Mover a la Papelera, Presentar (modo pantalla completa tipo diapositivas), Disponible sin conexión, Texto pequeño, Ancho completo, Personalizar página, Bloquear página (evitar ediciones por accidente), Usar con la IA, Sugerir ediciones, Traducir, Importar, Exportar, Convertir en wiki, Notificaciones y estadísticas, Historial de versiones, Comentarios/Conexiones.

**Construidas, las dos que se habían marcado como más simples y útiles de entrada:**
- **Ancho completo:** el contenido ocupa todo el ancho disponible en vez de la columna angosta de siempre — guardado por página (`full_width` en la tabla `pages`, migración `011_page_display_options.sql`)
- **Bloquear página:** protección simple contra ediciones por accidente — no es un permiso real entre usuarios distintos (eso no aplica todavía, Glenwyn es de un usuario por cuenta). Un candado visible en la barra superior cuando está activo. Neutraliza todos los handlers que modifican contenido en un solo lugar (`Block.jsx`), en vez de tocar cada tipo de bloque por separado

**Construidas también, en una tanda posterior:**
- **Copiar enlace:** genera un link con `#page=<id>` — Glenwyn no usa routing por URL para navegación interna (todo vive en `activeId`), así que en vez de construir un sistema de rutas completo solo para esto, se usa un hash liviano que la app ya sabe leer al cargar y navegar a esa página automáticamente
- **Copiar contenido:** copia la página al portapapeles como Markdown, reutilizando la misma conversión que ya usa "Exportar" — solo que al portapapeles en vez de descargar un archivo
- Ambas disponibles tanto en el menú "⋯" de la barra superior como en el menú "⋯" de cada fila del árbol (idea #59), con un aviso visual chico ("Enlace copiado" / "Contenido copiado") que se desvanece solo

**Construida también — Mover a:**
- Modal con buscador para elegir un nuevo padre para la página (o "Sin página superior" para llevarla al nivel principal), reutilizando el mismo patrón de búsqueda que ya existía para vincular páginas (`PageLinkBlock`)
- Filtra la propia página y todos sus descendientes de la lista de destinos posibles — moverla dentro de sí misma o de un hijo suyo crearía un ciclo imposible de recorrer
- Disponible en los dos menús "⋯" (barra superior y cada fila del árbol)

**Bug crítico encontrado y arreglado al construir esto:** la función nueva se llamó `movePage` al principio — mismo nombre que una función ya existente e importada de `pageUtils.js` (la que reordena páginas al arrastrar y soltar en el árbol). Como mi definición vivía dentro del mismo componente, tapaba silenciosamente a la importada para el resto de esa función — el arrastrar-y-soltar hubiera quedado roto (llamando a la función equivocada, con los argumentos en el orden equivocado) sin ningún error de compilación que lo avisara. Encontrado por el lint (avisó que la importación había quedado "sin usar", la pista de que algo la estaba tapando) antes de llegar a producción. Renombrada a `moveToNewParent`.

**Construida también — Personalizar página:**
- **Estilo de fuente** (Por defecto/Serif/Mono) y **Texto pequeño**, guardados por página (`font_style`, `small_text` en `pages`, migración `012_page_personalization.sql`)
- En vez de tocar cada `fontFamily` puesta a mano en `Block.jsx`/`SpecializedBlocks.jsx` (son muchas, repartidas entre varios tipos de bloque), se sobreescribe con `!important` desde un contenedor con la clase correspondiente más arriba en el árbol — mucho menos riesgoso que auditar cada bloque uno por uno
- Estas son preferencias de lectura, no ediciones de contenido — no se ven afectadas por "Bloquear página" (igual que el modo claro/oscuro)

**Pendiente, cosmético:** la vista pública compartida (`SharedPageView`) no respeta todavía el estilo de fuente/texto pequeño de la página original — queda con su propio estilo fijo. Candidato para una pasada de consistencia, no urgente.

**Sigue pendiente, sin evaluar todavía:** Presentar, Convertir en wiki, y el resto de la lista original — quedan para una próxima pasada, sin copiar todo por copiar.

**Auditoría de interacciones (julio 2026) — 4 huecos reales encontrados y arreglados, todos por no haber llegado a cubrir bases de datos al construir estas dos opciones:**
1. **"Bloquear página" no bloqueaba nada en una base de datos** — `DatabaseView` nunca revisaba `page.locked`, así que se podían seguir agregando/editando/eliminando registros y propiedades igual. Arreglado con el mismo patrón de `Block.jsx`: todos los handlers que mutan contenido, neutralizados en un solo lugar
2. **"Ancho completo" no tenía ningún efecto en una base de datos** — el ancho estaba fijo en 920px sin importar la preferencia guardada. El toggle aparecía con su marca de verificado, pero visualmente no pasaba nada. Arreglado para que `fullWidth` tenga prioridad sobre el ancho fijo de bases de datos
3. **"Restaurar" en el historial de versiones ignoraba el bloqueo** — reemplazaba todo el contenido de una página igual, aunque estuviera bloqueada. Es exactamente el tipo de edición que "Bloquear página" debería evitar. Arreglado con una alerta clara y el botón visualmente deshabilitado cuando corresponde
4. **Duplicar una página bloqueada bloqueaba la copia también** — inconveniente, ya que el propósito de duplicar suele ser tener una copia editable enseguida. Arreglado con el mismo razonamiento que ya se aplicaba a `pinned`/`shareToken` al duplicar

**Pendiente, cosmético, no bloqueante:** el input de celda de una tabla y el botón "+ agregar propiedad" no muestran un estado visual deshabilitado cuando la página está bloqueada — la protección funciona (no se guarda nada), pero se siente raro poder "escribir" en un campo que en realidad no va a guardar el cambio. Candidato para una pasada de pulido visual, no de lógica.
