...
### 1. Base de datos
Andá al **SQL Editor** de tu proyecto en supabase.com y corré, en orden, `001_init.sql`, `002_pinned.sql`, `003_page_versions.sql`, `004_storage.sql` y `005_sharing.sql`. El primero crea la tabla `pages` con Row Level Security habilitado (cada usuario solo ve sus propias páginas); el segundo agrega la columna `pinned` para favoritos; el tercero crea la tabla de historial de versiones; el cuarto crea el bucket de Storage para subir imágenes de verdad; el quinto habilita compartir páginas por link de solo lectura.
