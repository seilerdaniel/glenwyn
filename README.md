...
- **Sidebar sin duplicados**: "Atajos de teclado" y "Guía de uso" vivían en 3 y 2 lugares distintos respectivamente (fila suelta + menú de la barra superior + Ajustes). Sacadas las filas sueltas — siguen 100% disponibles desde Ajustes, el menú "⋯", el atajo `?`, y la paleta de comandos. El footer del sidebar bajó de 7 filas a 4 (más una quinta duplicación encontrada después: "Cerrar sesión")
- **Ancho completo y Bloquear página**: dos opciones nuevas en el menú "⋯" de la barra superior — ancho completo usa todo el espacio horizontal disponible en vez de la columna angosta de siempre; bloquear página impide ediciones por accidente (protección simple, no un permiso real entre usuarios). Un candado visible junto al título cuando está bloqueada
- **Menú "⋯" de cada página, ya no se corta**: vivía dentro del contenedor con scroll del árbol, así que se recortaba cuando la fila estaba cerca del final visible. Ahora se renderiza en un portal directo en `document.body`, con posición calculada — nunca más se corta, y se abre hacia arriba solo si no hay espacio suficiente en toda la ventana
